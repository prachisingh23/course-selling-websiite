...
const sanitizeGlobals = (source) => Object.fromEntries(
	Object.entries(source).map(([name, value]) => [name.trim(), value]),
);

export default [
	{ ignores: ['node_modules/**', 'dist/**', 'build/**', 'vite.config.js', 'vite.config.js.timestamp-*.mjs'] },
	{
		files: ['**/*.js', '**/*.jsx'],
		...
		languageOptions: {
			...
			globals: { ...sanitizeGlobals(globals.browser), React: 'readonly', Intl: 'readonly' },
		},
		settings: {
	...
	{ files: ['tools/**/*.js', 'tailwind.config.js'], languageOptions: { globals: sanitizeGlobals(globals.node) } },
];
